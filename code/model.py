...
class TimeNet(nn.Module):
    ...
    def __init__(self, n_forecasts, n_lags=0, n_changepoints=0, trend_smoothness=0):
        ...
        if self.n_changepoints > 0:
            if self.segmentwise_trend:
                self.trend_k = new_param(dims=[self.n_changepoints + 1]) # if without k0
            else:
    ...
    def _segmentwise_trend(self, t):
        ...
        if self.continuous_trend:
            ...
            deltas = self.trend_k[:] - torch.cat((self.trend_k0, self.trend_k[0:-1]))
    ...
    def trend(self, t):
        if int(self.n_changepoints) == 0:
            ...
            if self.segmentwise_trend:
                return self._segmentwise_trend(t)
            else:
                return self._deltawise_trend(t)
