...
class UnitTests(unittest.TestCase):
    ...
    def test_n_lags_and_n_regressors(self):
        ...
        def n_lags_only():
            ...
            log.debug('Testing n_lags>0, n_regressors=0 and n_forecasts>1')
            m = NeuralProphet(n_forecasts=2,n_lags=10,yearly_seasonality=False,
            weekly_seasonality=False,daily_seasonality=False)
            print(' n_lags: ',m.n_lags),print(' n_regressors: ',m.n_regressors),print(' n_forecasts: ',m.n_forecasts)
        ...
        def n_lags_plus_n_regressors():
            ...
            m = NeuralProphet(n_forecasts=2,n_lags=10,yearly_seasonality=False,
            weekly_seasonality=False,daily_seasonality=False)
            m = m.add_lagged_regressor(names='A',n_regressors=5)
            print(' n_lags: ',m.n_lags),print(' n_regressors: ',m.n_regressors),print(' n_forecasts: ',m.n_forecasts)
            ...
            log.debug('Testing n_lags=0 and n_regressors>0 - n_forecasts>1')
            m = NeuralProphet(n_forecasts=2,n_lags=0,yearly_seasonality=False,
            weekly_seasonality=False,daily_seasonality=False)
            m = m.add_lagged_regressor(names='A',n_regressors=3)
            print(' n_lags: ',m.n_lags),print(' n_regressors: ',m.n_regressors),print(' n_forecasts: ',m.n_forecasts)
            metrics = m.fit(df1, freq="D")
            future = m.make_future_dataframe(df2,n_historic_predictions=True)
            forecast = m.predict(df=future)
