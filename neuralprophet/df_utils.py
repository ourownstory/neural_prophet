...
def _check_dataframe(df, check_y, covariates, regressors, events, trend):
    ...
    Args:
        ...
        trend (pd.DataFrame): columns "cap" and "floor"
...
def check_dataframe(df, check_y=True, covariates=None, regressors=None, events=None, trend=None):
    ...
    checked_df = list()
    for df in df_list:
        checked_df.append(_check_dataframe(df, check_y, covariates, regressors, events, trend))
    df = checked_df
