...
class TimeNet(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.config_trend.growth in ["linear", "discontinuous"]:
            ...
            self.w_trend = new_param_weights(dims=[1,self.n_forecasts,1])
        ...
        if self.season_dims is not None:
            ...
            self.w_seas = new_param_weights(dims=[1,self.n_forecasts,1])
        ...
        if self.n_lags > 0:
            ...
            self.w_ar = new_param_weights(dims=[1,self.n_forecasts,1])
        ...
        if self.config_lagged_regressors is not None:
            ...
            for lay in self.covar_net:
                nn.init.kaiming_normal_(lay.weight, mode="fan_in")
            self.w_lag_reg = new_param_weights(dims=[1,self.n_forecasts,1])
    ...
    def forward(self, inputs, meta=None):
        ...
        self.w_trend = nn.Parameter(softmax(self.w_trend))
        self.w_ar = nn.Parameter(softmax(self.w_ar))
        self.w_lag_reg = nn.Parameter(softmax(self.w_lag_reg))
        self.w_seas = nn.Parameter(softmax(self.w_seas))


        self.w_trend = nn.Parameter(self.w_trend + 1 - (1/4))
        self.w_ar = nn.Parameter(self.w_ar + 1 - (1/4))
        self.w_lag_reg = nn.Parameter(self.w_lag_reg + 1 - (1/4))
        self.w_seas = nn.Parameter(self.w_seas + 1 - (1/4))


        out = (
            trend*self.w_trend
            + self.auto_regression(lags=inputs["lags"])*self.w_ar
            + self.forward_covar_net(covariates=inputs["covariates"])*self.w_lag_reg
            + self.all_seasonalities(s=inputs["seasonalities"], meta=meta)*self.w_seas
            #+ additive_components
