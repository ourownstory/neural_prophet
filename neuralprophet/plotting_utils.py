...
def plot_components(m, fcst, forecast_in_focus=None, figsize=None):
    ...
    if m.covar_config is not None:
        for name in m.covar_config.keys():
            if forecast_in_focus is None:
                components.append(
                    {
                        "plot_name": 'Covariate "{}"'.format(name),
    ...
    if "events_additive" in fcst.columns:
        components.append(
            {"plot_name": "Additive Events", "comp_name": "events_additive"}
        )
    ...
    for ax, comp in zip(axes, components):
        ...
        if (
            ...
            if m.season_config.mode == "multiplicative":
                multiplicative_axes.append(ax)
            plot_forecast_component(fcst=fcst, ax=ax, **comp)
        elif "auto-regression" in name or "covariate" in name or "residuals" in name:
...
def plot_forecast_component(
    ...
    if rolling is not None:
        rolling_avg = (
            fcst[comp_name].rolling(rolling, min_periods=1, center=True).mean()
        )
        if bar:
            artists += ax.bar(
                fcst_t, rolling_avg, width=1.00, color="#0072B2", alpha=0.5
            )
        else:
    ...
    if num_overplot is not None:
        ...
        for i in list(range(num_overplot))[::-1]:
            ...
            if bar:
                ...
                artists += ax.plot(
                    fcst_t[notnull], y[notnull], ls="-", color="#0072B2", alpha=alpha
                )
...
def plot_parameters(
    m, forecast_in_focus=None, weekly_start=0, yearly_start=0, figsize=None
):
    ...
    if m.country_holidays_config is not None:
        for country_holiday in m.country_holidays_config["holiday_names"]:
            ...
            weight_list = [
                (key, param.detach().numpy()) for key, param in event_params.items()
            ]
    ...
    if m.covar_config is not None:
        for name in m.covar_config.keys():
            if m.covar_config[name].as_scalar:
                additive_scalar_regressors.append(
    ...
    for ax, comp in zip(axes, components):
        ...
        if plot_name.startswith("trend"):
            ...
            if name.lower() == "weekly" or m.season_config.periods[name]["period"] == 7:
                ...
                name.lower() == "yearly"
                or m.season_config.periods[name]["period"] == 365.25
            ):
...
def plot_trend(m, ax=None, plot_name="Trend", figsize=(10, 6)):
    ...
    if m.n_changepoints == 0:
        ...
        artists += ax.plot(
            df_y["ds"].dt.to_pydatetime(), df_trend["trend"], ls="-", c="#0072B2"
        )
    # Specify formatting to workaround matplotlib issue #12925
    locator = AutoDateLocator(interval_multiples=False)
    formatter = AutoDateFormatter(locator)
